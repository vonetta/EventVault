...
/**
 * Shared media-upload handler used by both the admin and team-uploader routes.
 * Callers are responsible for authenticating the request first; `allowedKinds`
 * restricts which media kinds this caller may create (uploaders only get the
 * shared galleries), and `actor` is recorded in the audit log.
 */
export async function processMediaUpload(
  ...
  if (!options.allowedKinds.includes(kindRaw as MediaKind)) {
